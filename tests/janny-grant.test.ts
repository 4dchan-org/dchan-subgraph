...
describe("Janny grant", () => {
    ...
    test("grant janny (by janny)", () => {
        ...
        const jsonMessage = `{"ns": "dchan","v": 0,"op": "janny:grant","data": {"board": "${boardId}", "user": "${to.toHexString()}"}}`
    ...
    test("grant janny (by user)", () => {
        ...
        const jsonMessage = `{"ns": "dchan","v": 0,"op": "janny:grant","data": {"board": "${boardId}", "user": "${from.toHexString()}"}}`
        const newMessageEvent = createMessageEvent(from, jsonMessage)
